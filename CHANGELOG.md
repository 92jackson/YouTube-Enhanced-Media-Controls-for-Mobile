...
- (Hopefully) Fixed issue with incorrect redirecting when Sticky Repeat is enabled and a new video is manually started from the search results.
...
- **Layout Updates** - Added the ability to customise the layout of the player controls. The layout is configurable in the options, each button can be customised.
- **Vertical and Horizontal Playlist Alignment** - Added an options to customise how the active items in playlists are aligned.
- **Repeat Persistence** - Added an option to persist the repeat state across videos when Repeat is enabled (page reloads and video changes).
- **Seek Back and Forward Buttons** - Added optional seek back and forward buttons to the player controls. The seek duration is configurable in the options.
- **Playlist Snapshots** - Added the ability to create "Snapshots" of Mixes/Playlists, this takes the mix currently playing and stores a local version of all items within that mix. This allows you to return to the exact same playlist at any time, even if the mix is no longer available on YouTube. The ability to create and restore snapshots is available in the Favorites dialog using the 'Add'. button. EXPERIMENTAL- while functional, any video changes trigger a full page reload rather than triggering YouTube's native SPA navigation.
    - **Snapshot Sorting** - Added the ability to sort items within a Snapshot, access via the context menu on the Favorites dialog.
- **Loop and Shuffle Buttons** - Added optional playlist loop and shuffle buttons to the playlist drawer header. The buttons are configurable in the options. Using these buttons will convert the current playlist into a Snapshot Playlist- this prevents native YouTube from modifying the playlist (adding and removing items) as this would break the loop/shuffle functionality.
- **Import/Export User Settings** - Added the ability to import and export user settings to a JSON file. This includes all options set, stored snapshots, blacklisted videos, and favourites. Accessible via the options page.
- **Change Log** - Added a change log to the popup page, this lists all recent notable changes to the project.

### Bugs

- **YouTube has increased it's efforts to block background play for unpaid users, currently I have been unable to implement a workaround for this, as a result it's likely that the Background Play option will not work for now. If anyone has a solution or workaround please let me know.**
